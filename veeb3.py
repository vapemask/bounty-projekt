...
# Aether Command Center HTML ja JS loogika, mis suhtleb nüüd reaalse API-ga
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="et">
<head>
...
<body class="bg-black text-gray-300 font-mono h-screen flex flex-col overflow-hidden">
    ...
    <script>
        const defaultCode = `import sys\\nimport os\\n\\ntarget = sys.argv[1] if len(sys.argv) > 1 else "määramata"\\nprint(f"[+] Alustan autonoomset analüüsi sihtmärgile: {target}")\\nprint("[*] Keskkond on turvaline ja privaatne. Serveri OS andmed:")\\nos.system("uname -a")\\nprint("[+] Testimine edukas!")`;
        ...
        async function executePayload() {
            ...
            try {
                // Leiame dünaamiliselt õige baasteekonna, arvestades Cloud Shelli proxy't
...
@app.route('/')
def index():
    # Tagastame peamise kasutajaliidese
...
def execute_code():
    ...
    # Privacy-First: Loome ajutise faili skripti jooksutamiseks, mis kustub pärast töö lõppu
    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as temp_script:
        temp_script.write(code)
        temp_script_path = temp_script.name

    try:
        # Käivitame kasutaja sisestatud koodi (turvatud Dockeri liivakastis)
        # Timeout on seatud 60 sekundi peale, et vältida hangumist
...
if __name__ == '__main__':
    # Kuulame porti 8080, et sinu Dockeri -p 8081:8080 jne kaardistused töötaksid korrektselt
    print("[*] Aether Command Center Backend käivitub...")
